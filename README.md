...
The platform is built upon three foundational pillars that manage the entire lifecycle of student intervention without adding administrative weight:

1. **Pillar 1: Discovery** — The Early Warning System (EWS) lives here. Instead of relying on rigid thresholds, it actively looks for **velocity patterns** (e.g., sudden absences) and **baseline anomalies** (e.g., standard deviation drops in grades) to automatically detect students who need specialized care. For a deep dive into how we extract rich data with zero added teacher workload, see [discovery.md](pillars/discovery.md).
2. **Pillar 2: Response** — Once a need is discovered, the system becomes an intelligent assistant. It matches interventions based on **historical success**, and provides **generative, context-aware check-in scripts** based on the specific student's data and NLP notes, allowing the teacher to focus purely on human connection. See [response.md](pillars/response.md).
3. **Pillar 3: Recovery** — Interventions don't happen in a vacuum. The system utilizes **shadow monitoring** (temporary high-sensitivity tracking) to catch early relapses, and automatically prompts a **"Success Handshake" (Positive Reinforcement)** when a student's baseline stabilizes, securely closing the loop. See [recovery.md](pillars/recovery.md).
